...
int mdns_socket_open_ipv4(int port)
{
...
int mdns_socket_setup_ipv4(int sock, int port)
{
	unsigned char ttl      = 1;
...
int mdns_socket_open_ipv6(int port)
{
...
int mdns_socket_setup_ipv6(int sock, int port)
{
	int hops               = 1;
	...
	size_t * ofs_rhs)
{
	...
	do
	{
		...
		if (strncasecmp(
			    ...
			    (const char *)buffer_lhs + lhs_substr.offset,
			    rhs_substr.length))
			return 0;
...
mdns_string_t mdns_string_extract(
	const void * buffer, size_t size, size_t * offset, char * str, size_t capacity)
{
	size_t cur = *offset;
	...
	void * user_data)
{
	...
	for (size_t i = 0; i < records; ++i)
	{
		...
		*offset += 10;
		...
		{
			++parsed;
			if (callback(from, type, rtype, rclass, ttl, buffer, size, *offset, length, user_data))
				do_callback = 0;
		}
...
int mdns_unicast_send(
	int sock, void * address, size_t address_size, const void * buffer, size_t size)
{
...
size_t mdns_discovery_recv(
	int sock, void * buffer, size_t capacity, mdns_record_callback_fn callback, void * user_data)
{
	...
	for (i = 0; i < answer_rrs; ++i)
	{
		...
		int is_answer = mdns_string_equal(
			buffer, data_size, &ofs, mdns_services_query, sizeof(mdns_services_query), &verify_ofs);
		data = (uint16_t *)((char *)buffer + ofs);
...
int mdns_discovery_listen(
	int sock, void * address, size_t * address_size, void * buffer, size_t capacity)
{
	...
	uint8_t one_shot)
{
	...
	size_t offset  = (size_t)((char *)data - (char *)buffer);
	records += mdns_records_parse(
		saddr, buffer, data_size, &offset, MDNS_ENTRYTYPE_ANSWER, answer_rrs, callback, user_data);
	records += mdns_records_parse(
		saddr,
		buffer,
		data_size,
...
mdns_record_srv_t mdns_record_parse_srv(
	const void * buffer,
	size_t size,
	size_t offset,
	size_t length,
...
struct sockaddr_in * mdns_record_parse_a(
	const void * buffer, size_t size, size_t offset, size_t length, struct sockaddr_in * addr)
{
	memset(addr, 0, sizeof(struct sockaddr_in));
	addr->sin_family = AF_INET;
#ifdef __APPLE__
...
struct sockaddr_in6 * mdns_record_parse_aaaa(
	const void * buffer, size_t size, size_t offset, size_t length, struct sockaddr_in6 * addr)
{
